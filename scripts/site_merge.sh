...
#                   master.
#
# Usage          :  site_merge.sh -i merge.ini [-merge] \
...
gIni=""
gMerge="no"
gCBModule="site_merge.cbs"
gCBRoutine="SiteMerge"
...
usage() {
  ...
  #
  echo "Site Merge"
  echo "Usage: site_merge.sh -i merge.ini [options]"
...
#
if [ $gMerge = "yes" ]; then
  echo "Running site merge..."
else
  echo "Creating site merge report..."
fi
cat <<! >clarify.env
login_name=$gUser
db_server=$gServer
...
auto_login=TRUE
!
$gCBBatch  -f $gCBModule -f merge.cbs -m $gCBModule -r $gCBRoutine >site_merge.$$.tmp 2>&1
rm -f clarify.env
...
# least got started.
#
logfile=$(grep "MERGEFILE:" site_merge.$$.tmp | (read a b c; echo $b))
if [ "$logfile" = "" ]; then
  #
  # Startup failure
  #
