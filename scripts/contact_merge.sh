...
#                                                                          
# Name           :  contact_merge.sh                                             
#                                                                           
# Description    :  Provides contact merge report to display current 
#                   master/alias status in a Clarify database. Additionally 
#                   provides merge function which reassigns appropriate 
#                   objects such as cases from the alias contact to the 
#                   master.
#
# Usage          :  contact_merge.sh -i merge.ini [-merge] \
...
#                   default, contact_merge.sh generates a report. You must specify 
...
usage() {
  ...
  #
  echo "Contact Merge"
  echo "Usage: contact_merge.sh -i merge.ini [options]"
...
#
if [ $gMerge = "yes" ]; then
  echo "Running contact merge..."
else
  echo "Creating contact merge report..."
fi
cat <<! >clarify.env
login_name=$gUser
db_server=$gServer
...
auto_login=TRUE
!
$gCBBatch -f $gCBModule -m $gCBModule -r $gCBRoutine >merge.$$.tmp 2>&1
rm -f clarify.env
...
# least got started.
#
logfile=$(grep "MERGEFILE:" merge.$$.tmp | (read a b c; echo $b))
if [ "$logfile" = "" ]; then
  #
  # Startup failure
  #
